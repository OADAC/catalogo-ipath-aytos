...
function imgTag(src,alt,cls='',fallbacks=[]){
  ...
  return `<img${cls?` class="${cls}"`:''} src="${esc(resolved)}" alt="${esc(alt)}"${resolvedFallbacks.length?` data-fallbacks="${esc(resolvedFallbacks.join('|'))}"`:''} decoding="async">`;
}
function isVideo(src=''){return /\.mp4(?:$|\?)/i.test(src)}
function mediaTag(src,alt,cls='',fallbacks=[]){
  if(isVideo(src)){const resolved=releaseAsset(src), resolvedFallbacks=releaseAssetList(fallbacks);return `<video${cls?` class="${cls}"`:''} src="${esc(resolved)}" aria-label="${esc(alt)}" muted loop autoplay playsinline preload="metadata"${resolvedFallbacks.length?` data-fallbacks="${esc(resolvedFallbacks.join('|'))}"`:''}></video>`;}
...
function renderTextures(){
  ...
  const t=TEXTURES[textureActive];const preview=$('#texturePreview');preview.src=releaseAsset(t.img);preview.dataset.fallbacks=releaseAssetList(t.fallbacks||[]).join('|');preview.dataset.fallbackIndex='0';$('#textureName').textContent=t.name;$('#textureUse').textContent=t.use;$('#textureSpec').innerHTML=`<b>${t.name}</b><p>${t.use} El acabado seleccionado queda asociado a la documentación del elemento para facilitar futuras reposiciones.</p><div class="chips">${t.chips.map(x=>`<span>${x}</span>`).join('')}</div>`;
}
