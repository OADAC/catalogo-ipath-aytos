...
const releaseAsset=value=>window.IPATH_RELEASE_ASSETS?.resolve(value)||value;
const releaseAssetList=values=>(window.IPATH_RELEASE_ASSETS?.resolveList(values)||values);
...
function imgTag(src,alt,cls='',fallbacks=[],options={}){
  const resolved=releaseAsset(src),resolvedFallbacks=releaseAssetList(fallbacks);
  const eager=options.eager===true;
  return `<img${cls?` class="${cls}"`:''} src="${esc(resolved)}" alt="${esc(alt)}"${resolvedFallbacks.length?` data-fallbacks="${esc(resolvedFallbacks.join('|'))}"`:''} loading="${eager?'eager':'lazy'}" fetchpriority="${eager?'high':'low'}" decoding="async">`;
}
function deferredImgTag(src,alt,cls='',fallbacks=[]){
  const resolved=releaseAsset(src),resolvedFallbacks=releaseAssetList(fallbacks);
  return `<img class="${esc([cls,'deferred-release-img'].filter(Boolean).join(' '))}" src="${TRANSPARENT_PIXEL}" data-deferred-src="${esc(resolved)}" alt="${esc(alt)}"${resolvedFallbacks.length?` data-fallbacks="${esc(resolvedFallbacks.join('|'))}"`:''} decoding="async">`;
}
let deferredObserver;
function activateDeferredImages(root=document){
  if(!deferredObserver){
    deferredObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{
      if(!entry.isIntersecting)return;
      const img=entry.target;img.src=img.dataset.deferredSrc;delete img.dataset.deferredSrc;
      img.onload=()=>img.classList.add('is-loaded');deferredObserver.unobserve(img);
    }),{root:null,rootMargin:'180px 0px',threshold:.01});
  }
  root.querySelectorAll?.('img[data-deferred-src]').forEach(img=>deferredObserver.observe(img));
}
function isVideo(src=''){return /\.mp4(?:$|\?)/i.test(src)}
function mediaTag(src,alt,cls='',fallbacks=[]){
  if(isVideo(src)){const resolved=releaseAsset(src), resolvedFallbacks=releaseAssetList(fallbacks);return `<video${cls?` class="${cls}"`:''} src="${esc(resolved)}" aria-label="${esc(alt)}" muted loop playsinline controls preload="none"${resolvedFallbacks.length?` data-fallbacks="${esc(resolvedFallbacks.join('|'))}"`:''}></video>`;}
  return imgTag(src,alt,cls,fallbacks,{eager:true});
}
document.addEventListener('error',e=>{
  ...
  if(idx<list.length){
    const next=releaseAsset(list[idx]);el.dataset.fallbackIndex=String(idx+1);
...
const APP_FALLBACKS={};
function appFallbacks(a){return [...new Set([...(a.fallbacks||[]),'assets/02_a310ae3a17.webp','assets/07_984c9222f8.webp'].filter(Boolean))]}
let appFilter='Todas',activeApp=0,activeMedia=0;
...
function renderFilters(){
  ...
  $$('#appFilters button').forEach(b=>b.onclick=()=>{appFilter=b.dataset.cat;renderFilters();renderApps();});
}
function renderApps(){
  const items=visibleApps();
  if(!items.some(x=>x.orig===activeApp)){activeApp=items[0]?.orig||0;activeMedia=0;}
  $('#appGrid').innerHTML=items.map(a=>`<article class="app-card ${a.orig===activeApp?'active':''}" data-i="${a.orig}"><div class="pic">${deferredImgTag(a.img,a.name,'',appFallbacks(a))}<span class="id">${String(a.orig+1).padStart(2,'0')}</span><h3>${a.name}</h3></div><div class="meta"><b>${a.cat}</b><span>${a.formats}</span></div></article>`).join('');
  $$('.app-card').forEach(c=>c.onclick=()=>selectApp(+c.dataset.i,false));
  activateDeferredImages($('#appGrid'));
...
/* ---------- Process ---------- */
const PROCESS=[
{name:'Diagnóstico',icon:'◎',img:'assets/process/01_preparacion.webp',fallback:'assets/02_a310ae3a17.webp',desc:'Necesidades, estado del suelo, servicios existentes y objetivos municipales.',deliver:'Alcance y línea base'},
{name:'Diseño',icon:'✎',img:'assets/process/01_preparacion.webp',fallback:'assets/02_a310ae3a17.webp',desc:'Solución física, funcional y digital adaptada al emplazamiento.',deliver:'Proyecto y configuración'},
{name:'Fabricación',icon:'▥',img:'assets/process/01_preparacion.webp',fallback:'assets/02_a310ae3a17.webp',desc:'Prefabricación controlada, componente circular, identidad y calidad.',deliver:'Lote documentado'},
{name:'Logística',icon:'▰',img:'assets/process/02_instalacion.webp',fallback:'assets/02_a310ae3a17.webp',desc:'Transporte, acopio y secuencia de instalación planificados.',deliver:'Entrega trazada'},
{name:'Instalación',icon:'◇',img:'assets/process/02_instalacion.webp',fallback:'assets/02_a310ae3a17.webp',desc:'Montaje seco, verificación y aceptación de cada zona o elemento.',deliver:'Prueba de instalación'},
{name:'Operación',icon:'▣',img:'assets/process/03_verificacion.webp',fallback:'assets/07_984c9222f8.webp',desc:'Inventario, incidencias, revisiones, mantenimiento e histórico.',deliver:'Activo visible'},
{name:'Reconfiguración',icon:'↻',img:'assets/process/03_verificacion.webp',fallback:'assets/07_984c9222f8.webp',desc:'Cambio de uso, ampliación o reforma sustituyendo componentes.',deliver:'Nueva configuración'},
{name:'Segundo uso',icon:'♻',img:'assets/process/03_verificacion.webp',fallback:'assets/07_984c9222f8.webp',desc:'Desmontaje, reutilización, retorno o reciclaje con destino documentado.',deliver:'Cierre circular'}
];
...
renderFilters();renderApps();renderProcess();restartProcessTimer();renderLayers();restartLayerTimer();renderFormats();renderBenefits();updateImpact();initNav();heroParallax();
